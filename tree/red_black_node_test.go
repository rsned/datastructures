...
// the node in the previous steps in the algorithm. If the node was not found
// this method isn't called.
func TestRotateRightRedBlack(t *testing.T) {
	tests := []struct {
		name         string
		setupTree    func() *redBlackNode[int]
		rotateNode   int
		...
		{
			//    5  ->  5
			name: "node_without_left_child_should_return_same_node",
			setupTree: func() *redBlackNode[int] {
				return &redBlackNode[int]{
					value:  5,
					isRed:  false,
		...
		},
		{
			...
			//  \       /
			//   7     7
			name: "right_rotation_with_right_subtree_becomes_right_with_left_subtree",
			setupTree: func() *redBlackNode[int] {
				// Create tree: 10 -> (5 -> 7)
				root := &redBlackNode[int]{
					value:  10,
		...
		},
		{
			...
			//  /        \
			// 5          10
			name: "right_rotation_with_parent_becomes_left_with_right_subtree",
			setupTree: func() *redBlackNode[int] {
				// Create tree: 15 -> (10 -> 5)
				parent := &redBlackNode[int]{
					value:  15,
		...
		},
		{
			...
			//  /        \
			// 5          7
			name: "right_rotation_of_node_with_parent_and_right_child_becomes_right_leaf_with_new_parent",
			setupTree: func() *redBlackNode[int] {
				// Create tree: 10 -> (7 -> 5)
				parent := &redBlackNode[int]{
					value:  10,
		...
		},
		{
			...
			//  /                 \
			// 5                   14
			name: "right_rotation_with_complex_parent_child_relationships",
			setupTree: func() *redBlackNode[int] {
				// Create tree: 15 -> (12 -> (10 -> 5))
				parent := &redBlackNode[int]{
					value:  15,
					isRed:  false,
					parent: nil,
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			newRoot := rotateRightRedBlack(nodeToRotate)
